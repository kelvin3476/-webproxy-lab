...
//   // exit 함수의 경우 통상적으로 정상적인 종료를 했을 경우 0 값을 반환하고, 오류로 인해 종료를 해야할 경우 0이 아닌 값들을 매개변수로 주는 것으로 알고 있기에
//   // 코드 문맥상 비정상적인 종료로 인해 exit을 사용하므로 매개변수로 1을 넣어주는게 좋다.
...
//   Rio_writen(clientfd, buf, (size_t)strlen(buf)); // => 적어주는 행위 자체가 요청하는것이다.
// }
